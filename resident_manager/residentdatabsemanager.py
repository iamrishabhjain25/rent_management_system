...
class ResidentManager:
    ...
    def _process_resident_exit(
        ...
        if not curr_status[self.uid].isin([row[self.uid]]).any():
            raise ValueError(
                f"Inlavid Exit. {self.uid}:{row[self.uid]} does not exist in status"
            )

        curr_bed = curr_status.loc[
            curr_status[self.uid] == row[self.uid], self.bed_id
        ].squeeze()
        if curr_bed != row[self.bed_id]:
            raise ValueError(
                f"Invalid Exit. The bedID of exiting {self.uid} does not match this uid in status"
            )
        ...
        row_with_calc["RentDays"] = (
            row_with_calc["RentThruDate"] - row_with_calc["LastRentCalcDate"]
        ).dt.days
        row_with_calc["RentDue"] = (
            row_with_calc["RentDays"] * row_with_calc["Rent"] / monthly_factor
        )
        row_with_calc["ElectricityCharges"] = (
            row_with_calc["CumulativeElectConsumption"] * self.confs.elect_rate
        )
        ...
        row_with_calc["NetAmountDue"] = (
            row_with_calc["TotalAmountDue"] - row_with_calc["Deposit"]
        )
        self.data_manager.insert_final_settlement_record(row_with_calc)
        return row_with_calc

    @log_and_backup()
    def _process_resident_entry(
        self, row: pd.Series, log_comments: Optional[str] = None
    ):
        ...
        uit_at_entering_bed = curr_status.loc[
            curr_status[self.bed_id] == row[self.bed_id], self.uid
        ].squeeze()
        if not pd.isna(uit_at_entering_bed):
            raise ValueError(
                f"Invalid Entry. The BedID {row[self.bed_id]} is not empty."
            )

        if not row[self.uid] in residents_info[self.uid].values:
            raise ValueError(
                f"{self.uid}: {row[self.uid]} does not exist in databse. "
                "Please create an admission or enter valid Enrollment ID"
            )

        if row[self.uid] in curr_status[self.uid].values:
            curr_room = curr_status[
                curr_status[self.uid] == row[self.uid], self.bed_id
            ].squeeze()
            raise ValueError(
                f"Invalid Entry. The Enrollment id is already staying in Bed {curr_room}"
            )
    ...
    def _electricity_adjustment(self, adj_type: str, row: pd.Series):
        ...
        occupied_room = pd.concat(
            [occupied_room, room_prev_vals[occupied_room.columns]]
        ).drop_duplicates(subset=[self.uid], keep="last")
        curr_status = pd.concat([empty_rooms, occupied_room])
        curr_status.loc[
            curr_status[self.room_id] == row[self.room_id], "RoomElectricityReading"
        ] = row["RoomElectricityReading"]
        ...
        if adj_type == "exit":
            ...
            curr_status.loc[
                curr_status[self.uid] == row[self.uid], cols_to_chng
            ] = np.nan
    ...
    def _pre_transaction_room_elect_adjustment(
        ...
        room_prev_vals["RoomElectricityConsumption"] = (
            room_prev_vals["NewRoomElectricityReading"]
            - room_prev_vals["RoomElectricityReading"]
        )
        room_prev_vals["CumulativeElectConsumption"] += room_prev_vals[
        ...
        room_prev_vals["PrevElectricityCalcDate"] = room_prev_vals[
            "LastElectricityCalcDate"
        ]
    ...
    def _source_room_exit(self, input_row: pd.Series):
        data = {
            ...
            "AdditionalCharges": self.data_manager.load_current_status(filter_bedId=input_row["SourceBedId"], filter_cols="AdditionalCharges").squeeze(),
    ...
    def _destination_room_exit(self, input_row: pd.Series):
        data = {
            ...
            "AdditionalCharges": self.data_manager.load_current_status(filter_bedId=input_row["DestinationBedId"], filter_cols="AdditionalCharges").squeeze(),
            "Comments": input_row["Comments"],
            "PaymentReceived": np.nan,
    ...
    def room_meter_change(
        ...
        occupied_room = curr_status[curr_status[self.uid].notna()]
        occupied_room = pd.concat(
            [occupied_room, room_adjusted[occupied_room.columns]]
        ).drop_duplicates(subset=[self.uid], keep="last")
        curr_status = pd.concat([empty_rooms, occupied_room])
        curr_status.loc[
            curr_status[self.room_id] == room, "RoomElectricityReading"
        ] = new_meter_reading
        self.data_manager.update_current_status(new_status=curr_status)
    ...
    def calculate_rent(
        ...
        prev_eom_rent_calc_date = eom_rent_calc_date.replace(day=1) - dt.timedelta(
            days=1
        )
        ...
        if (occupied_beds["LastRentCalcDate"] < prev_eom_rent_calc_date).any():
            raise ValueError(
                f"Previous month's rent is not yet settled. "
                f"Still have LastRentCalcDate before {prev_eom_rent_calc_date}"
            )
        ...
        occupied_beds["RoomElectricityConsumption"] = (
            occupied_beds["EOMElectReading"] - occupied_beds["RoomElectricityReading"]
        )
        occupied_beds["CumulativeElectConsumption"] += occupied_beds[
            "RoomElectricityConsumption"
        ] / occupied_beds.groupby(self.room_id)["RoomElectricityConsumption"].transform(
            "count"
        )
        occupied_beds["PrevElectricityReading"] = occupied_beds[
            "RoomElectricityReading"
        ]
        ...
        occupied_beds["ElectricityCharges"] = (
            occupied_beds["UnitsConsumed"] * self.confs.elect_rate
        )
        occupied_beds["PrevElectricityCalcDate"] = occupied_beds[
            "LastElectricityCalcDate"
        ]
    ...
    @log_and_backup()
    def update_and_save_rent_calculation(
        self, new_status: pd.DataFrame, rent_history: pd.DataFrame
    ):
