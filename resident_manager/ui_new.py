...
class ResidenceManagementStreamlit:
    ...
    def contact_input(self, label: str, value=None, length: int = 10, key:str = None):
    ...
    def new_admission(self):
        ...
        data = {
            ...
            "DateofBirth": st.date_input("Date of Birth", min_value=datetime.strptime("1950-01-01","%Y-%m-%d"), format="DD-MM-YYYY"),
            ...
            "Address": st.text_input("Full Address", value="Write Full address with pincode cit and all details"),
        ...
        if (data["Rent"] < 5000).any() or (data["Deposit"] < data["Rent"] * 2 ).any():
            message = "Warning: Either Rent if less than 5,000 or Deposite is less than twice the rent. Are you sure want to continue ?"
    ...
    def new_electricity_record(self):
        ...
        if invalid_reading.any():
            st.warning(f"Invalid Electricity Reading : Reading for ({invalid_reading[invalid_reading==1].index.tolist()}) is less than previous reading. cannot enter this record")
            return
    ...
    def update_resident_info(self):
        ...
        if st.session_state.resident_data is not None:
            ...
            updated_data = {
                ...
                "DateofAdmission": st.date_input("Date of Admission", value=resident_data.get("DateofAdmission").date()),
                ...
                "AdditionalContact": self.contact_input("Additional Contact Number", value=resident_data.get("AdditionalContact")),
                ...
                "InstituteContactOther": self.contact_input("Institute Other Contact", value=resident_data.get("InstituteContactOther")),
                ...
                "GuardianRelation": st.text_input("Relation with Guardian", value=resident_data.get("GuardianRelation")),
    ...
    def update_electricity_record(self):
        ...
        if st.button("Load Info"):
            if not update_dt:
                raise st.warning(f"Please enter date to load details and update")
        ...
        if st.session_state.prev_record is not None:
            ...
            updated_data.update({
            "Meter_1_2A": st.number_input("Enter Main Meter 1 Reading", value=prev_record.get("Meter_1_2A")),
            "Meter_2_2B": st.number_input("Enter Main Meter 2 Reading", value=prev_record.get("Meter_2_2B")),
            "Meter_3_1A": st.number_input("Enter Main Meter 3 Reading", value=prev_record.get("Meter_3_1A")),
            "Meter_4_1B": st.number_input("Enter Main Meter 4 Reading", value=prev_record.get("Meter_4_1B")),
            "Meter_5_GA": st.number_input("Enter Main Meter 5 Reading", value=prev_record.get("Meter_5_GA")),
            "Meter_6_GB": st.number_input("Enter Main Meter 6 Reading", value=prev_record.get("Meter_6_GB")),
            "Meter_7_Basement": st.number_input("Enter Main Meter 7 Reading", value=prev_record.get("Meter_7_Basement")),
    ...
    def record_activity(self):
        ...
        trans_elect = st.number_input("Enter room electricity reading at the time of exit/entry", value=last_elect_reading)
        if trans_elect < last_elect_reading:
            error += f"Invalid Electricity Reading : Current transaction reading ({trans_elect}) cannot be less than last electricity reading ({last_elect_reading})"
        ...
        display_df = display_df.merge(residets_info[[self.db_manager.uid, "Name","Rent", "Deposit"]], on=self.db_manager.uid, how="left")
        display_df = display_df[["TransDate","RentThruDate", "BedID", "EnrollmentID", "Name", "Rent", "Deposit",
                                 "TransType", "RoomElectricityReading", "PrevDueAmount", "AdditionalCharges", "Comments" ]]
    ...
    def view_current_tables(self):
        ...
        match table_name:
            case self.db_manager.confs.residents_tbl:
                ...
                df[self.db_manager.confs.date_cols_residents_tbl] = df[self.db_manager.confs.date_cols_residents_tbl].apply(lambda x : x.dt.strftime("%d-%b-%Y"))
            ...
            case self.db_manager.confs.electricity_tbl:
                ...
                df[self.db_manager.confs.date_cols_electricity_tbl] = df[self.db_manager.confs.date_cols_electricity_tbl].apply(lambda x : x.dt.strftime("%d-%b-%Y %H:%M:%S"))
                df = df.T

            case self.db_manager.confs.transactions_tbl:
                df = self.db_manager.data_manager.load_transactions_table()
                df = df.merge(residents_info[["EnrollmentID", "Name"]],on=self.db_manager.uid, how="left")

                df[self.db_manager.confs.date_cols_transactions_tbl] = df[self.db_manager.confs.date_cols_transactions_tbl].apply(lambda x : x.dt.strftime("%d-%b-%Y"))

            case self.db_manager.confs.final_settlement_tbl:
                df = self.db_manager.data_manager.load_final_settlement_table()
                df[self.db_manager.confs.date_cols_final_settlement_tbl] = df[self.db_manager.confs.date_cols_final_settlement_tbl].apply(lambda x : x.dt.strftime("%d-%b-%Y"))

            case self.db_manager.confs.rent_history_tbl:
                df = self.db_manager.data_manager.load_rent_history()
                df[self.db_manager.confs.date_cols_rent_history_tbl] = df[self.db_manager.confs.date_cols_rent_history_tbl].apply(lambda x : x.dt.strftime("%d-%b-%Y"))

            case self.db_manager.confs.current_status_tbl:
                df = self.db_manager.data_manager.load_current_status()
                df = df.merge(
                    residents_info[["EnrollmentID", "Name", "ContactNumber", "OtherContact", "FathersContact", "MothersContact", "Course" ]],
                    on=self.db_manager.uid, how="left")

                df[self.db_manager.confs.date_cols_status_tbl] = df[self.db_manager.confs.date_cols_status_tbl].apply(lambda x : x.dt.strftime("%d-%b-%Y"))
                df[self.db_manager.room_id] = np.where(df[self.db_manager.room_id].isna(), np.nan, df[self.db_manager.room_id].astype(int))
...
def main():
    ...
    menu = ["Save a Copy" ,"New Admission", "New Electricity Reading", "Update Resident Info", "Update Electricity Record",
            "Entry/Exit of Form", "View Current Tables", "Calculate Rent", "Electricity Meter Change", "Undo Last change" ]
    ...
    if choice == "Undo Last change":
        ...
        db_list = [file.strip(db_manager.db_handler.db_extension) for file in files_list if (file.endswith(db_manager.db_handler.db_extension) and file.startswith(db_manager.db_handler.db_filename))]
...
if __name__ == "__main__":
    ...
    # if "show_undo_warning" not in st.session_state:
    #     st.session_state.show_undo_warning = False

    # if st.sidebar.button("Undo Recent Changes"):
    #     st.session_state.show_undo_warning = True

    # if st.session_state.show_undo_warning:
    #     with st.sidebar:
    #         st.warning("Are you sure you want to undo recent changes? This will revert to the last available database.")
    #         col1, col2 = st.columns(2)
    #         with col1:
    #             if st.button("Confirm Undo"):
    #                 try:
    #                     # Implement your logic to revert the database here
    #                     db_manager.data_manager.revert_to_last_backup()
    #                     st.success("Reverted to the last available database.")
    #                     st.session_state.show_undo_warning = False
    #                 except Exception as e:
    #                     st.error(f"Failed to undo changes: {e}")
    #         with col2:
