...
class ResidenceManagementStreamlit:
    ...
    def contact_input(self, label: str, value=None, length: int = 10, key: Optional[str] = None):
        ...
        if len(contact) != length:
            st.error(
                f"length of input ({len(contact)}) does not match required length: ({length})"
            )
    ...
    def new_admission(self):
        ...
        data = {
            ...
            "DateofAdmission": st.date_input(
                "Date of Admission", value=datetime.today(), format="DD-MM-YYYY"
            ),
            ...
            "AdditionalContact": self.contact_input(
                "Addtional Contact Number (10 digit)"
            ),
            ...
            "InstituteContact": self.contact_input(
                "Institute Contact Number (10 digit)"
            ),
            "InstituteContactOther": self.contact_input(
                "Institute other Contact Number (10 digit)"
            ),
            ...
            "GuardianContact1": self.contact_input(
                "Guardian Contact Number (10 digit)"
            ),
            "GuardianContact2": self.contact_input(
                "Guardian other Contact Number (10 digit)"
            ),
        ...
        if st.button("Submit Admission"):
            try:
                self.db_manager.data_manager.insert_resident_record(
                    input_df=data, log_comments=log_comments
                )
    ...
    def new_electricity_record(self):
        ...
        elect_table = self.db_manager.data_manager.load_electricity_table().sort_values(
            ["Date"]
        )
        curr_status = self.db_manager.data_manager.load_current_status()
        curr_reading_status = (
            curr_status[["RoomNo", "RoomElectricityReading"]]
            .drop_duplicates()
            .set_index("RoomNo")
            .squeeze()
        )
        ...
        reading_date = st.date_input(
            "Electricity Reading Date", value=datetime.now().date(), format="DD-MM-YYYY"
        )
        ...
        prev_record = (
            elect_table[elect_table["Date"] < reading_datetime].tail(1).squeeze()
        )
        ...
        for a_room in rooms:
            data[a_room] = st.number_input(
                f"Enter Room {a_room} Meter Reading", value=prev_record[a_room]
            )

        data.update(
            {
                "Meter_1_2A": st.number_input(
                    "Enter Main Meter 1 Reading", value=prev_record["Meter_1_2A"]
                ),
                "Meter_2_2B": st.number_input(
                    "Enter Main Meter 2 Reading", value=prev_record["Meter_2_2B"]
                ),
                "Meter_3_1A": st.number_input(
                    "Enter Main Meter 3 Reading", value=prev_record["Meter_3_1A"]
                ),
                "Meter_4_1B": st.number_input(
                    "Enter Main Meter 4 Reading", value=prev_record["Meter_4_1B"]
                ),
                "Meter_5_GA": st.number_input(
                    "Enter Main Meter 5 Reading", value=prev_record["Meter_5_GA"]
                ),
                "Meter_6_GB": st.number_input(
                    "Enter Main Meter 6 Reading", value=prev_record["Meter_6_GB"]
                ),
                "Meter_7_Basement": st.number_input(
                    "Enter Main Meter 7 Reading", value=prev_record["Meter_7_Basement"]
                ),
                "Library": st.number_input(
                    "Enter Library Meter Reading", value=prev_record["Library"]
                ),
                "Solar": st.number_input(
                    "Enter Solar Meter Reading", value=prev_record["Solar"]
                ),
            }
        )
        ...
        if st.button("Submit Readings"):
            try:
                self.db_manager.data_manager.insert_electricity_record(
                    input_df=data, log_comments=log_comments
                )
    ...
    def update_resident_info(self):
        ...
        if st.button("Load Info"):
            if not uid:
                raise ValueError(
                    f"Please enter {self.db_manager.uid} to load resident details and update"
                )

            resident_data = self.db_manager.data_manager.load_residents_info_table(
                filter_ids=[uid]
            )
            str_cols_to_fill = resident_data.select_dtypes(
                include=["string", "O"]
            ).columns
        ...
        if st.session_state.resident_data is not None:
            ...
            updated_data = {
                "EnrollmentID": st.text_input(
                    "Enrollment ID", value=uid, disabled=True
                ),
                "BedID": st.text_input(
                    "Select Bed ID", value=resident_data.loc[self.db_manager.bed_id]
                ),
                "DateofAdmission": st.date_input(
                    "Date of Admission",
                ...
                "DateofBirth": st.date_input(
                    "Date of Birth", value=resident_data.get("DateofBirth").date()
                ),
                "FathersName": st.text_input(
                    "Father's Full Name", value=resident_data.get("FathersName")
                ),
                "ContactNumber": self.contact_input(
                    "Contact Number", value=resident_data.get("ContactNumber")
                ),
                "OtherContact": self.contact_input(
                    "Other Contact Number", value=resident_data.get("OtherContact")
                ),
                "FathersContact": self.contact_input(
                    "Father's Contact Number", value=resident_data.get("FathersContact")
                ),
                "MothersContact": self.contact_input(
                    "Mother's Contact Number", value=resident_data.get("MothersContact")
                ),
                "AdditionalContact": self.contact_input(
                ...
                "Address": st.text_input(
                    "Full Address", value=resident_data.get("Address")
                ),
                "BloodGroup": st.text_input(
                    "Blood Group", value=resident_data.get("BloodGroup")
                ),
                "ResidentAadhar": st.text_input(
                    "Resident Aadhaar", value=resident_data.get("ResidentAadhar")
                ),
                "FathersAadhar": st.text_input(
                    "Father's Aadhaar", value=resident_data.get("FathersAadhar")
                ),
                "InstituteName": st.text_input(
                    "Institute Name", value=resident_data.get("InstituteName")
                ),
                "InstituteAddress": st.text_input(
                    "Institute Address", value=resident_data.get("InstituteAddress")
                ),
                "Course": st.text_input(
                    "Course Name (JEE/NEET/IAS)", value=resident_data.get("Course")
                ),
                "Batch": st.text_input("Batch", value=resident_data.get("Batch")),
                "InstituteContact": self.contact_input(
                    "Institute Contact", value=resident_data.get("InstituteContact")
                ),
                "InstituteContactOther": self.contact_input(
                    "Institute Other Contact",
                    value=resident_data.get("InstituteContactOther"),
                ),
                "InstituteID": st.text_input(
                    "Institute ID Number", value=resident_data.get("InstituteID")
                ),
                "GuardianName": st.text_input(
                    "Name of Guardian", value=resident_data.get("GuardianName")
                ),
                ...
                "GuardianContact1": self.contact_input(
                    "Guardian Contact 1", value=resident_data.get("GuardianContact1")
                ),
                "GuardianContact2": self.contact_input(
                    "Guardian Contact 2", value=resident_data.get("GuardianContact2")
                ),
                "GuardianAddress": st.text_input(
                    "Guardian Address", value=resident_data.get("GuardianAddress")
                ),
                "Rent": st.number_input(
                    "Monthly Rent", value=resident_data.get("Rent")
                ),
                "Deposit": st.number_input(
                    "Deposit", value=resident_data.get("Deposit")
                ),
                "RentStartDate": st.date_input(
                    "Rent Start Date", value=resident_data.get("RentStartDate").date()
                ),
            }

            updated_data_df = pd.DataFrame([updated_data])

            if (updated_data_df["Rent"] < 5000).any() or (
                updated_data_df["Deposit"] < updated_data_df["Rent"] * 2
            ).any():
                message = "Warning: Either Rent if less than 5,000 or"
                "Deposite is less than twice the rent. Are you sure want to continue ?"
                st.warning(message, icon="⚠️")
            ...
            if st.button("Update"):
                try:
                    self.db_manager.data_manager.edit_resident_record(
                        input_df=updated_data_df, log_comments=log_comments
                    )
    ...
    def update_electricity_record(self):
        ...
        elect_records = (
            self.db_manager.data_manager.load_electricity_table().sort_values(["Date"])
        )
        date_map = {}
        ...
        update_dt = st.selectbox(
            "Choose the date to update", options=list(date_map.keys())
        )
        if st.button("Load Info"):
            if not update_dt:
                raise st.warning("Please enter date to load details and update")

            prev_record = elect_records[
                elect_records["Date"] == date_map[update_dt]
            ].squeeze()
            latest_before_dt = (
                elect_records[elect_records["Date"] < update_dt].tail(1).squeeze()
            )
        ...
        if st.session_state.prev_record is not None:
            ...
            rooms = (
                pd.Series(self.db_manager.confs.valid_bedIDs)
            ...
            for a_room in rooms:
                updated_data[a_room] = st.number_input(
                    f"Enter Room {a_room} Meter Reading", value=prev_record.get(a_room)
                )
            ...
            updated_data.update(
                {
                    ...
                    "Library": st.number_input(
                        "Enter Library Meter Reading", value=prev_record.get("Library")
                    ),
                    "Solar": st.number_input(
                        "Enter Solar Meter Reading", value=prev_record.get("Solar")
                    ),
                }
            )
            ...
            if st.button("Update"):
                try:
                    self.db_manager.data_manager.edit_electricity_record(
                        input_df=updated_data_df, log_comments=log_comments
                    )
    ...
    def record_activity(self):
        ...
        if bed_id:
            last_elect_reading = curr_status.loc[
                curr_status[self.db_manager.bed_id] == bed_id, "RoomElectricityReading"
            ].squeeze()
        ...
        if trans_type == "entry":
            ...
            name = ""
            if uid:
                matching_resident = residets_info[
                    residets_info[self.db_manager.uid] == uid
                ]
        ...
        if trans_type == "exit":
            ...
            if not matching_row.empty:
                ...
                name = residets_info.loc[
                    residets_info[self.db_manager.uid] == uid, "Name"
                ].squeeze()
            else:
                uid = None
                name = None
            uid = st.text_input(
                "Enrollment ID of exiting resident", value=uid, disabled=True
            )
            name = st.text_input("Name of exiting resident", value=name, disabled=True)
            prev_due = st.number_input(
                "Enter any previous rent/electricity due amount.", value=0
            )
            additional_charges = st.number_input(
                "Enter any additional charges", value=0
            )
            rent_thru_dt = st.date_input(
                "Charge Rent until", value=datetime.now().date(), format="DD-MM-YYYY"
            )
            rent_thru_dt = rent_thru_dt.strftime("%d-%b-%Y")
            comments = st.text_input(
                "Enter any comments or remarks.", value="No Comments"
            )

        trans_dt = st.date_input(
            "Enter Exit/Entry Date", value=datetime.now().date(), format="DD-MM-YYYY"
        )
    ...
    def view_current_tables(self):
        ...
        if table_name in [self.db_manager.confs.current_status_tbl]:
            st.subheader(
                f"Empty Bed Count : {len(self.db_manager.data_manager.get_empty_beds())}"
            )
        ...
        match table_name:
            case self.db_manager.confs.residents_tbl:
                ...
                df[self.db_manager.confs.date_cols_residents_tbl] = df[
                    self.db_manager.confs.date_cols_residents_tbl
            ...
            case self.db_manager.confs.electricity_tbl:
                ...
                df[self.db_manager.confs.date_cols_electricity_tbl] = df[
                    self.db_manager.confs.date_cols_electricity_tbl
                ].apply(lambda x: x.dt.strftime("%d-%b-%Y %H:%M:%S"))
                df = df.T
            ...
            case self.db_manager.confs.transactions_tbl:
                ...
                df[self.db_manager.confs.date_cols_transactions_tbl] = df[
                    self.db_manager.confs.date_cols_transactions_tbl
                ].apply(lambda x: x.dt.strftime("%d-%b-%Y"))

            case self.db_manager.confs.final_settlement_tbl:
                df = self.db_manager.data_manager.load_final_settlement_table()
                df[self.db_manager.confs.date_cols_final_settlement_tbl] = df[
                    self.db_manager.confs.date_cols_final_settlement_tbl
                ].apply(lambda x: x.dt.strftime("%d-%b-%Y"))

            case self.db_manager.confs.rent_history_tbl:
                df = self.db_manager.data_manager.load_rent_history()
                df[self.db_manager.confs.date_cols_rent_history_tbl] = df[
                    self.db_manager.confs.date_cols_rent_history_tbl
                ].apply(lambda x: x.dt.strftime("%d-%b-%Y"))
            ...
            case self.db_manager.confs.current_status_tbl:
                ...
                df[self.db_manager.confs.date_cols_status_tbl] = df[
                    self.db_manager.confs.date_cols_status_tbl
                ].apply(lambda x: x.dt.strftime("%d-%b-%Y"))
                df[self.db_manager.room_id] = np.where(
                    df[self.db_manager.room_id].isna(),
                    np.nan,
    ...
    def change_electricity_meter(self):
        ...
        prev_reading = curr_status.loc[
            curr_status[self.db_manager.room_id] == room, "RoomElectricityReading"
        ].iloc[0]

        trans_dt = st.date_input(
            "Enter meter change  Date", value=datetime.now().date(), format="DD-MM-YYYY"
        )
        ...
        old_meter_reading = st.number_input(
            "Enter Old Meter electricity reading", value=prev_reading
        )
        if prev_reading > old_meter_reading:
            error += "\n Invalid Reading. Old reading cannot be less than previous reading of the meter"

        new_meter_reading = st.number_input(
            "Enter New Meter Starting electricity reading", value=0
        )
    ...
    def calculate_monthly_rent(self):
        ...
        dt_input = st.date_input(
            "Enter month End Date", value=closer_dt, format="DD-MM-YYYY"
        )
        st.warning("Choose only the month end date like 31Oct, 28Feb, 30Jun", icon="⚠️")
        update_and_save = st.selectbox(
            "Update Database and Status ?", options=["No", "Yes"]
        )
...
def main():
    ...
    db_manager = RDM.ResidentManager(
        confs=configs, db_handler=db_handler, data_manager=data_manager
    )  # Replace this with your actual db manager
    ...
    st.write(
        f"Curretn Databse in Use -> {db_handler.get_latest_db_path(full_path=False)}"
    )
    ...
    if choice == "Undo Last change":
        ...
        db_list = [
            ...
            if (
                file.endswith(db_manager.db_handler.db_extension)
                and file.startswith(db_manager.db_handler.db_filename)
            )
        ]
        ...
        if st.session_state.undo_warning_displayed:
            st.warning(
                "Are you sure you want to undo recent changes? This action cannot be undone."
            )
            ...
            with col1:
                if st.button("Confirm Undo"):
                    try:
                        ...
                        st.success(
                            "Reverted to the last available backup successfully."
                        )
                        st.session_state.undo_warning_displayed = False
                    except Exception as e:
